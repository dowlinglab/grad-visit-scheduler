...
Applied when automatic break constraints are enabled (`movement` legacy
`NO_OFFSET` compatibility or `enforce_breaks` set to `True` / a positive
integer):
...
- `min_faculty=1`
- `max_group=2`
- `enforce_breaks=True` (or a positive integer such as `2`)
